...
def generate_html_output(json_files):
    ...
    for json_file in json_files:
        ...
        if json_file.endswith('.html'):
            # Special class for db_output.html iframe to make it larger
            iframe_class = 'max-iframe' if 'db_output.html' in json_file else 'fullpage'
            html_content += f'<div id="{json_name}" class="tabcontent"><iframe class="{iframe_class}" src="{json_file}"></iframe></div>'
        else:
    ...
    html_content += '''
        ...
        <script>
            ...
            // Initialize the first tab and adjust iframe height
